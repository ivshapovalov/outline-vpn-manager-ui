...
import {
    ...
    MESSAGE_SERVER_GET_PROPERTIES_FAILED,
...
export async function DOMContentLoaded() {
    ...
    const serverPane = document.getElementById('pills_server_tab');
    serverPane.addEventListener('shown.bs.tab', () => {
    ...
    serverPane.addEventListener('hidden.bs.tab', () => {
...
function handleServerPropertyInputBytes(event) {
...
function handleKeyInputBytes(event) {
    let input = event.currentTarget;
    let inputId = input.id;
    input.value = formatNumber(input.value);
    ...
    if (new DataByte(keysFiltered.find((el) => el.id === keyId)[keyProperty]).equals(new DataByte(totalBytes))) {
        [buttonSave, buttonCancel].forEach((button) => undisplayElement(button))
    } else {
        if (isTotalBytesValid(inputId)) {
            [buttonSave, buttonCancel].forEach(button => displayElement(button))
...
function createServerProperties() {
    let serverWrapper = document.getElementById('server_properties');
    ...
    for (let property of Object.keys(SERVER_PROPERTIES)) {
        ...
        if (SERVER_PROPERTIES[property].type === 'checkbox') {
            ...
            inputWrapper.classList.add('input-group-text', 'input-group-checkbox-server');
...
export async function getServerProperties() {
    ...
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_SERVER_GET_PROPERTIES, options)
            .then(function (response) {
                    if (response.ok) {
                        return response.json().then(function (properties) {
                            serverProperties = {};
                            for (let property of Object.keys(SERVER_PROPERTIES)) {
        ...
        console.error(error);
        if (error.stack) {
            showToastError(MESSAGE_SERVER_GET_PROPERTIES_FAILED);
        } else {
...
async function getKeys() {
    ...
    let options = {method: "GET", headers: headers};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEY_LIST, options)
            .then(function (response) {
                    if (response.ok) {
        ...
        console.error(error);
        if (error.stack) {
            showToastError(MESSAGE_KEY_LIST_FAILED);
        } else {
            showToastError(error);
        }
    } finally {
...
function setKeyLimit(event) {
    ...
    input.addEventListener('input', (event) => handleKeyInputBytes(event))
...
function appendButtons(fieldId, functions) {
    ...
    if (functions.remove) {
        ...
        btnRemove.classList.add('input-group-btn', 'input-group-btn-cancel');
...
export async function removeKey(event) {
    ...
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEY_REMOVE, options)
            .then(function (response) {
                    if (response.ok) {
                        const currentKey = keysFiltered.find((key) => key.id === keyId);
                        const currentKeyIndex = keysFiltered.indexOf(currentKey)
...
export async function setKeyProperty(id, property, newValue) {
    ...
    let options = {method: "POST", headers: headers, body: data};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEY_SET_PROPERTY, options)
            .then(function (response) {
                    if (response.ok) {
...
export async function removeKeyProperty(id, property) {
    ...
    let options = {method: "POST", headers: headers, body: data};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEY_REMOVE_PROPERTY, options)
            .then(function (response) {
                    if (response.ok) {
                        let currentKey = keys.find((el) => el.id === Number(id));
                        let inputId = 'key_' + id + '_' + property;
...
export async function removeKeysCustomLimits() {
    ...
    let options = {method: "POST", headers: headers, body: data};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEYS_REMOVE_CUSTOM_LIMITS, options)
...
export async function addKey() {
    ...
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_KEY_ADD, options)
            .then(function (response) {
                    if (response.ok) {
                        return response.json().then(function (json) {
                            showToastMessage(MESSAGE_KEY_ADD_SUCCESS + " " + json.id, TOAST_MESSAGE_DELAY_SEC)
                            keys.push(json);
                            updateKeys()
                            updateCurrentPageNumber();
                            updateKeysPage();
...
export async function setServerProperty(property, newValue) {
    ...
    let options = {method: "POST", headers: headers, body: data};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_SERVER_SET_PROPERTY, options)
            .then(function (response) {
                    if (response.ok) {
...
export async function removeServerProperty(property) {
    ...
    let options = {method: "POST", headers: headers, body: data};
    try {
        await fetch(MANAGER_URL + "/?action=" + ACTION_SERVER_REMOVE_PROPERTY, options)
            .then(function (response) {
                    if (response.ok) {
...
const SERVER_PROPERTIES = {
    id: {name: 'ID', title: ' Server ID', type: 'text', disabled: true, required: false},
    name: {
        name: 'Name',
        title: 'Server name',
        type: 'text',
    ...
    version: {name: 'Version', title: 'Server version', type: 'text', disabled: true},
    port: {
        name: 'Port',
    ...
    limit: {
        ...
        onInput: handleServerPropertyInputBytes
    }
}
